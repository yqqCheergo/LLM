...
# 训练示例
def train_gpt2(model, dataloader, criterion, optimizer, device):
    model.train()
    total_loss = 0
...
# 创建模型
model = GPT2(vocab_size, d_model, num_heads, num_layers, max_seq_length, d_ff).to(device)
...
for epoch in range(num_epochs):
    loss = train_gpt2(model, dataloader, criterion, optimizer, device)
